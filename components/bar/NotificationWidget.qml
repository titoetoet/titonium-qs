...
Item {
    ...
    readonly property bool expanded: root.screen ? (NotificationService.activeScreen === root.screen) : NotificationService.open
    readonly property Item inputRegion: inputArea

    readonly property int drawerWidth: 360
    readonly property int drawerHeight: Math.round((root.screen?.height || 1080) * 0.6)
    ...
    Item {
        ...
        x: root.expanded ? panel.x : 0
        y: 0
        width: root.expanded ? Math.max(root.width - panel.x, panel.width) : root.width
        height: root.expanded ? (root.panelTop + panel.height) : root.height
    }
    ...
    PopupPanel {
        ...
        x: root.parent ? (root.parent.width - root.drawerWidth - Theme.barContentMargin) - root.x : 0
        y: root.panelTop
        ...
        ColumnLayout {
            ...
            Item {
                ...
                Rectangle {
                    ...
                    // Sliding indicator
                    Rectangle {
                        x: root.currentTab === 0 ? 2 : parent.width / 2
                        y: 2
